...
<section id=enroll-container>
	...
	<h1>카트에 오신것을 환영합니다.</h1>
	<h2>나의 카트</h2>
	<form name="cartUpdateFrm" method="post" action="<%=request.getContextPath()%>/cart/myCartUpdate">
		<table>
			<tr>
				<th>아이디<sup>*</sup></th>
		...
		<input type="submit" value="정보수정" /> <input type="button" onclick="deleteCart();" value="삭제" />
	</form>
</section>

<form action="<%=request.getContextPath()%>/cart/myCartDelete" method="POST" name="cartDeleteFrm"></form>
<script>
const deleteCart = () => {
	if(confirm('정말 회원탈퇴하시겠습니까?')){
		document.cartDeleteFrm.submit();
	}
};
...
document.cartUpdateFrm.onsubmit = (e) => {
